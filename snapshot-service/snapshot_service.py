...
class SnapshotService:
    ...
    async def start_continuous_snapshots(self):
        ...
        logger.info(
            f"Snapshot interval: {self.config.SNAPSHOT_INTERVAL} seconds"
        )
        ...
        while True:
            try:
                if await self._check_qdrant_health():
                    ...
                    logger.warning(
                        "Qdrant is not available, skipping snapshot"
                    )
    ...
    async def _check_qdrant_health(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.config.QDRANT_URL}/health", timeout=5
            )
    ...
    async def _create_snapshot(self) -> Optional[SnapshotInfo]:
        ...
        if snapshot_info:
            logger.info(
                f"Filesystem snapshot created: {snapshot_info.filename}"
            )
            self.last_snapshot_time = snapshot_info.created_at
            return snapshot_info
    ...
    async def _create_api_snapshot(
        self, timestamp: str
    ...
    async def _create_filesystem_snapshot(
        self, timestamp: str
    ) -> Optional[SnapshotInfo]:
...
async def download_snapshot(filename: str):
    ...
    return FileResponse(
        path=filepath, filename=filename, media_type="application/gzip"
    )
...
async def restore_snapshot(filename: str):
    ...
    if not success:
        raise HTTPException(
            status_code=404, detail="Snapshot not found or restore failed"
        )
