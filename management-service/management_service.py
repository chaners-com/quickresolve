...
class ManagementService:
        ...
        try:
            ...
            if not containers:
                return ServiceStatus(
                    name=service_name, status="not_found", health=False
                )
            ...
            return ServiceStatus(
                name=service_name, status=status, health=health, uptime=uptime
            )
        except Exception as e:
            logger.error(f"Error getting status for {service_name}: {e}")
            return ServiceStatus(
                name=service_name, status="unknown", health=False
            )
    ...
    async def graceful_shutdown(self) -> ShutdownResponse:
        ...
        if self.shutdown_in_progress:
            raise HTTPException(
                status_code=409, detail="Shutdown already in progress"
            )
        ...
        try:
            ...
            for service_name in self.config.SERVICES:
                try:
                    ...
                    timeout = self.config.SHUTDOWN_TIMEOUTS.get(
                        service_name, 30
                    )
                    ...
                    if not containers:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Container for {service_name} not found"
                            )
                            continue
                    ...
                    if containers:
                        ...
                        logger.warning(
                            f"No container found for {service_name}"
                        )
                        continue
            ...
            logger.info(
                f"Graceful shutdown completed in {duration:.2f} seconds"
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Shutdown failed: {str(e)}"
            )
        finally:
            self.shutdown_in_progress = False
...
async def start_services(services: Optional[List[str]] = None):
    ...
    return {"success": True, "message": "Services started", "results": results}
...
async def restart_service(service_name: str):
    ...
    if success:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to restart {service_name}"
        )
